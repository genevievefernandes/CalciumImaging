function [densityData, densityNoise, xev, yev] = NoiseModel(deltaFoF, sigma, movements)
...
transfDataMatrix(isinf(transfDataMatrix)|isnan(transfDataMatrix)) = 0;
...
% Replace NaN or Infinite values with 0
data = [pointRow pointCol];
data(isinf(data)|isnan(data)) = 1;

hist2dim=accumarray(data, 1, [nevs nevs]);
