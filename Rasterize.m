function [raster, mapOfOddsJoint]=Rasterize(deltaFoF, sigma, movements, mapOfOdds, xev, yev, fps, tauDecay)
...
for numNeuron=1:size(transfDataMatrix,2)
    ...
    for numFrame=3:size(transfDataMatrix,1)-2
        % Index sanity check added
        nfPlusOne = bins(numFrame+1);
        nf = bins(numFrame);
        nfMinusOne = bins(numFrame-1);
        nfPlusTwo = bins(numFrame+2);
        nfMinusTwo = bins(numFrame-2);

        if (nf > 0 && nfPlusOne > 0 && nfMinusOne > 0 && nfPlusTwo > 0 && nfMinusTwo > 0)
